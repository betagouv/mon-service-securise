// ⚠️ Code dupliqué
// La logique de validation du MDP est codée côté front et côté back.
...
const valideMotDePasse = (motDePasse) => {
  ...
  if (motDePasse.length < 12) return resultatValidation.ERREUR_MOT_DE_PASSE_TROP_COURT;
  if (!motDePasse.match(/[A-Z]/)) return resultatValidation.ERREUR_PAS_DE_MAJUSCULE;
  if (!motDePasse.match(/[a-z]/)) return resultatValidation.ERREUR_PAS_DE_MINUSCULE;
  if (!motDePasse.match(/[0-9]/)) return resultatValidation.ERREUR_PAS_DE_CHIFFRE;
  if (!motDePasse.match(/[#?!@$%^&*'+_()[\]-]/)) return resultatValidation.ERREUR_PAS_DE_CARACTERE_SPECIAL;
