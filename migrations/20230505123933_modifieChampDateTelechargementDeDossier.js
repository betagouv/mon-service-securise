...
const modifieVersDateUnique = (knex, table) =>
  pourChaqueLigne(knex(table), ({ id, donnees }) => {
    ...
    donnees.dossiers = donnees.dossiers.map((dos) => {
      ...
      const telechargementsComplets =
        Object.keys(dos.datesTelechargements ?? {}).length === 3;
      if (telechargementsComplets) {
        const lesDates = Object.values(dos.datesTelechargements).map(
          (d) => new Date(d)
        );
...
const supprimeDateUnique = (knex, table) =>
  pourChaqueLigne(knex(table), ({ id, donnees }) => {
    if (!donnees.dossiers) return Promise.resolve();
