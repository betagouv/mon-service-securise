...
describe('Les statistiques sur les mesures de sécurité', () => {
  ...
  const verifieEgaliteNumerique = (valeurAttendue, valeurCalculee) => {
    ...
    try {
      const difference = Number(
        parseFloat(Math.abs(valeurAttendue - valeurCalculee)).toPrecision(
          nbDecimales + 1
        )
      );
      ...
      throw new Error(
        `Échec ! On voulait ${valeurAttendue}, mais on a eu ${valeurCalculee}.`
      );
    }
  };
  ...
  elles('calculent le score par catégorie', () => {
    ...
    expect(referentiel.coefficientIndiceCyberMesuresIndispensables()).to.equal(
      0.8
    );
    expect(referentiel.coefficientIndiceCyberMesuresRecommandees()).to.equal(
      0.2
    );
  ...
  describe("Lorsqu'il n'y a pas de mesure recommandée personnalisée", () => {
    elles(
      'calculent le score en ne tenant compte que des mesures indispensables',
      () => {
        const stats = new StatistiquesMesures(
          {
            une: {
  ...
  describe("Lorsqu'il n'y a pas de mesure indispensable personnalisée", () => {
    elles(
      'calculent le score en ne tenant compte que des mesures recommandées',
      () => {
        const stats = new StatistiquesMesures(
          {
            une: {
