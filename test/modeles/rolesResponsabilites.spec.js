...
describe("L'ensemble des rôles et responsabilités", () => {
  ...
  describe('sur une demande de description des responsables', () => {
    it("intègre l'autorité d'homologation", () => {
      const rolesResponsabilites = new RolesResponsabilites({
        autoriteHomologation: 'Jean Dupont', fonctionAutoriteHomologation: 'Maire',
      });

      const responsables = rolesResponsabilites.descriptionResponsables();
      expect(responsables).to.eql([{ role: "Autorité d'homologation", description: 'Jean Dupont (Maire)' }]);
    });
    ...
    it('intègre le ou la spécialiste cybersécurité', () => {
      ...
      const responsables = rolesResponsabilites.descriptionResponsables();
      expect(responsables).to.eql([{ role: 'Spécialiste cybersécurité', description: 'John Dupond (Spécialiste)' }]);
    });
    ...
    it('intègre le délégué ou la délégué à la protection des données à caractère personnel', () => {
      ...
      const responsables = rolesResponsabilites.descriptionResponsables();
      expect(responsables).to.eql([{ role: 'Délégué(e) à la protection des données à caractère personnel', description: 'Marie Age (Déléguée)' }]);
    });
    ...
    it('intègre le ou la responsable métier du projet', () => {
      ...
      const responsables = rolesResponsabilites.descriptionResponsables();
      expect(responsables).to.eql([{ role: 'Responsable métier du projet', description: 'Otto Graf (Pilote)' }]);
    });
    ...
    it('intègre les acteurs spécifiques', () => {
      ...
      const responsables = rolesResponsabilites.descriptionResponsables();
      expect(responsables).to.eql([{ role: 'Rôle dans le projet', description: 'Sandra Nicouverture (Fonction)' }]);
    });
    ...
    it('intègre les acteurs spécifiques sans les fonctions si elles ne sont pas présentes', () => {
      ...
      const responsables = rolesResponsabilites.descriptionResponsables();
      expect(responsables).to.eql([{ role: 'Rôle dans le projet', description: 'Yamamoto Kaderate' }]);
    });

    it("n'intègre pas les acteurs non renseignés", () => {
      const rolesResponsabilites = new RolesResponsabilites({});

      expect(rolesResponsabilites.descriptionResponsables()).to.have.length(0);
    });
  });
});
