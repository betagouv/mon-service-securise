  ...
  describe('quand requête POST sur /api/service/:id/homologation/finalise', () => {
    beforeEach(() => {
      ...
      testeur
        .middleware()
        .reinitialise({ homologationARenvoyer: homologationAvecDossier });
      testeur.depotDonnees().enregistreFinalisationHomologation = () =>
        Promise.resolve();
    });
    ...
    it("utilise le dépôt pour finaliser l'homologation", async () => {
      let servicePasse = {};
      testeur.depotDonnees().enregistreFinalisationHomologation = (service) => {
      ...
      expect(servicePasse.dossiers.items[0].finalise).to.be(true);
    });
  });
