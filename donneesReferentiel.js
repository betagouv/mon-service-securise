...
module.exports = {
  ...
  mesures: {
    ...
    formaliserSecu: {
      description: 'Formaliser les modalités de mise en oeuvre des mécanismes de sécurité importants pour le service',
      categorie: 'gouvernance',
    ...
    analyseRisque: {
      description: "Organiser au moins un atelier d'analyse des risque du service numérique",
      categorie: 'gouvernance',
    ...
    accesSecurise: {
      ...
      categorie: 'protection',
      indispensable: true,
      descriptionLongue: "Cette mesure permet d'éviter des mots de passe trop simples, trop courts ou anciens ne soient utilisés par un acteur malveillant pour accéder au service. Cette mesure peut être mise en oeuvre en fixant des règles au moment de l'enregistrement d'un mot de passe (longueur, complexité, etc.) ; l'introduction d'un testeur de force de mot de passe ;  un délai d'expiration du mot de passe ; une politique de renouvellement automatique ; un accès sans mot de passe par la réception d'un code à usage unique à chaque connexion (hors SMS) ; l'impossibilité de réutiliser des mots de passe déjà employés par le passé ; la révocation immédiate de mots de passe  en cas de connexion suspecte, etc. Pour plus de détails, consultez le <a href=‘guide de l'ANSSI relatives à l'authentification multifacteur et aux mots de passe.’>https://www.ssi.gouv.fr/administration/guide/recommandations-relatives-a-lauthentification-multifacteur-et-aux-mots-de-passe/</a>",
    },
    franceConnect: {
    ...
    compartimenter: {
      ...
      categorie: 'protection',
      indispensable: true,
      descriptionLongue: "Cette mesure permet de limiter la portée d'une compromission d'un compte d'administrateur et le risque de pouvoir accéder à des services/fonctions non nécessaires à ce rôle. Elle consiste à compartimenter les rôles d'administration métier/fonctionnel (rôle de publication, gestion des utilisateurs du sites, etc.) des rôles techniques (administration des bases de données, le serveur web, l'infrastructure d'accueil, l'admin de la ou des VM participants à l'offre de service).",
    },
    ...
    limitationRolesAdmin: {
      description: "Limiter les droits d'administration au seul rôle ciblé",
      categorie: 'protection',
      indispensable: true,
      descriptionLongue: "Cette mesure permet de reduire le pouvoir d'administration aux seuls services utiles à ce rôle. Par exemple, l'admin de base de donnée n'a de droits que sur la BDD, ou sur le serveur de bases de données, etc.",
    },
    ...
    securisationCode: {
      description: 'Mettre en oeuvre des bonnes pratiques de sécurisation du code',
      categorie: 'protection',
      indispensable: true,
    ...
    configurationMinimaliste: {
     ...
     verificationAutomatique: {
      ...
      categorie: 'protection',
      indispensable: true,
      descriptionLongue: "Cette mesure vise à vérifier, avant le lancement du service, la présence défauts de configuration ou de vulnérabilités connues, grâce à des outils de tests disponibles la plupart du temps gratuitement en ligne ou à installer en local comme <a href=‘testSSL permettant d’évaluer le niveau de confiance du certificat SSL’>https://testssl.sh/</a> ; <a href=‘MozzilaObservatory’ permettant d’évaluer le respect des bonnes pratiques de configuration http’>https://observatory.mozilla.org/</a> ; dependabot, codeQL, Nmap permettant d'identifier la présence de vulnérabilités.",
    },
     testIntrusion: {
    ...
    journalAcces: {
      description: "Mettre en oeuvre la journalisation et la centralisation des accès",
      categorie: 'defense',
      indispensable: true,
    ...
    journalEvenementSecu: {
      description: 'Mettre en oeuvre la journalisation et la centralisation des évènements de sécurité',
      categorie: 'defense',
      indispensable: true,
    ...
    affichageDerniereConnexion: {
      ...
      categorie: 'defense',
      descriptionLongue: 'Cette mesure contribuer à aider les utilisateurs à détecter eux-mêmes une utilisation à leur insu du service par une autre personne.',
    },
